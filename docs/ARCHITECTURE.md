...
### 🛡️ Топ-3 Стратегії Мінімізації Затримки (Latency Mitigation)

Для вирішення проблеми затримки у Entropia 3D та подібних системах застосовуються наступні інженерні стратегії:

#### 1. SharedArrayBuffer (Zero-Latency Shared Memory)
Замість передачі даних через `postMessage` (навіть Transferables), обидва потоки працюють з **спільною ділянкою пам'яті**.
...
- **Перевага**: Повна відсутність затримки копіювання чи передачі.
- **Ризики**: Потребує використання `Atomics` для запобігання race conditions та специфічних HTTP-заголовків (Cross-Origin Isolation).
