...
const SettingsPanel: React.FC<SettingsPanelProps> = ({ engine }) => {
  ...
  const Slider = ({ label, value, min, max, step, param, colorClass = "accent-emerald-500" }: { 
    ...
    <div className="group flex flex-col gap-1.5 mb-4 last:mb-0">
      <div className="flex justify-between items-center text-[10px] uppercase tracking-widest text-gray-400 group-hover:text-white transition-colors">
        <span className="font-bold">{label}</span>
        <span className="font-mono font-black text-white bg-white/5 px-2 py-0.5 rounded text-[9px] min-w-[36px] text-center">
      ...
      <input 
        ...
        className={`w-full h-1 bg-white/10 rounded-lg appearance-none cursor-pointer ${colorClass} hover:bg-white/20 transition-all`}
      />
    </div>
  );
  ...
  return (
    <div className="bg-white/5 rounded-2xl border border-white/5 shadow-2xl flex flex-col overflow-hidden pointer-events-auto">
      ...
      <div 
        ...
        className="px-5 py-4 flex justify-between items-center cursor-pointer hover:bg-white/5 transition-all group border-b border-white/5"
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[7px] text-gray-600 uppercase tracking-widest font-black -mt-0.5">Параметри пісочниці</div>
          </div>
        </div>
        <div className="flex flex-col items-end">
          <span className="text-[9px] text-emerald-500/80 font-mono font-black px-2 py-0.5 bg-emerald-500/10 rounded border border-emerald-500/20">v2.5</span>
        </div>
      </div>
      ...
      <div className={`transition-all duration-500 ease-in-out ${collapsed ? 'max-h-0 opacity-0' : 'max-h-[1600px] opacity-100'}`}>
        <div className="p-5 space-y-9 overflow-y-auto custom-scrollbar max-h-[60vh]">
          ...
          <section>
            <h3 className="text-[9px] text-emerald-400 font-black uppercase tracking-[0.4em] mb-4 flex items-center gap-3">
          ...
          {/* Group 2: AI / Boids */}
          <section>
            <h3 className="text-[9px] text-blue-400 font-black uppercase tracking-[0.4em] mb-4 flex items-center gap-3">
          ...
          <section>
            <h3 className="text-[9px] text-amber-400 font-black uppercase tracking-[0.4em] mb-4 flex items-center gap-3">
