...
export class Logger {
  ...
  private context: string = resolveRuntimeEnvironment() === 'worker' ? 'Worker' : 'Main';
  private commandSubscribers: Set<(command: RemoteCommand) => void> = new Set();
  ...
  public setRemoteLogging(enabled: boolean): void {
    ...
    if (this.remoteLoggingEnabled === nextState) {
      return;
    }

    this.remoteLoggingEnabled = nextState;
