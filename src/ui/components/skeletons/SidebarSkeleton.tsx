...
const SkeletonStats = () => (
  <div className="grid grid-cols-2 gap-3">
    {[...Array(UI_CONFIG.SKELETON.COUNTS.STATS)].map((_, i) => (
      <div
        key={i}
        className="bg-gray-800/30 rounded-xl p-4 border border-white/5 space-y-3 animate-pulse"
        style={{ animationDelay: `${i * UI_CONFIG.SKELETON.ANIMATION.DELAY_STEP_STATS}ms` }}
      >
      ...
      <div className="absolute inset-0">
        <svg className="w-full h-full opacity-20">
          ...
          <path
            ...
            strokeWidth="2"
            className="animate-pulse"
            style={{ animationDelay: `${UI_CONFIG.SKELETON.ANIMATION.DELAY_POPULATION_SECONDARY}ms` }}
          />
        </svg>
      </div>
    </div>
...
const SkeletonTelemetry = () => (
  <div className="space-y-3">
    {[...Array(UI_CONFIG.SKELETON.COUNTS.TELEMETRY)].map((_, i) => (
      <div
        key={i}
        className="flex items-center justify-between bg-gray-800/20 rounded-lg p-3 animate-pulse"
        style={{ animationDelay: `${i * UI_CONFIG.SKELETON.ANIMATION.DELAY_STEP_TELEMETRY}ms` }}
      >
