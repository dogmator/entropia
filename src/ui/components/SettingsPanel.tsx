...
const PanelHeader = ({ isCollapsed, onToggle }: { isCollapsed: boolean; onToggle: () => void }) => (
  <button
    type="button"
    onClick={onToggle}
  ...
  >
    <div className="flex items-center gap-3">
      <div className={`p-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 transition-all duration-300 ${isCollapsed ? '-rotate-90 scale-90 opacity-50' : 'rotate-0'}`}>
      ...
      <div>
        ...
        <div className="text-[7px] text-gray-600 uppercase tracking-widest font-black -mt-0.5">{t.settings.environment}</div>
      </div>
    </div>
  ...
  if (isLoading) {
    return (
      <div className="bg-white/5 rounded-2xl border border-white/5 p-4 flex items-center justify-center animate-pulse">
        <div className="text-gray-400 text-xs font-mono uppercase tracking-widest">{t.settings.loading}</div>
      </div>
    );
  }
  ...
  return (
    <SettingsProvider value={contextValue}>
      <div className="bg-white/5 rounded-2xl border border-white/5 shadow-2xl flex flex-col overflow-hidden pointer-events-auto">
        <PanelHeader isCollapsed={isCollapsed} onToggle={() => { setCollapsed(!isCollapsed); }} />
        <div className={`transition-all duration-500 ease-in-out ${isCollapsed ? 'max-h-0 opacity-0' : 'max-h-[1600px] opacity-100'}`}>
