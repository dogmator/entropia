...
export class Logger {
   ...
   */
  public warning(message: string, source?: string, data?: any): void {
   ...
   */
  private addLog(level: LogLevel, message: string, source?: string, data?: any): void {
    // Перевірка на дублювання логів (оптимізація)
    ...
    // Оптимізоване обмеження кількості логів
    if (this.logs.length > this.maxLogs) {
      // Видаляємо відразу 10% логів для кращої продуктивності
      const removeCount = Math.max(1, Math.floor(this.maxLogs * 0.1));
    ...
    // Вивід в консоль для відладки (тільки для error та warning)
    if (level === LogLevel.ERROR || level === LogLevel.WARNING) {
      const logMessage = `[${level.toUpperCase()}]${source ? ` [${source}]` : ''}: ${message}`;
      if (data) {
        console[level === LogLevel.ERROR ? 'error' : 'warn'](logMessage, data);
      } else {
        console[level === LogLevel.ERROR ? 'error' : 'warn'](logMessage);
      }
    }
  }
  ...
  public logPerformance(operation: string, duration: number, source?: string): void {
    const level = duration > 100 ? LogLevel.WARNING : LogLevel.INFO;
   ...
   */
  public logSimulationEvent(event: string, data?: any): void {
  ...
  public getLogStats(): {
    ...
    const recent = this.getRecentLogs(5).length;
