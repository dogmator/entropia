...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({ engine, worldScale, onWorldScaleChange }) => {
  ...
  /**
   * Допоміжний компонент для рендерингу типізованого слайдера.
   */
      ...
      <div className={`transition-all duration-500 ease-in-out ${collapsed ? 'max-h-0 opacity-0' : 'max-h-[1600px] opacity-100'}`}>
        <div className="p-4 sm:p-5 space-y-9 overflow-y-auto custom-scrollbar max-h-[60vh]">
          ...
          <section>
            ...
            <Slider label="Генерація їжі" param="foodSpawnRate" value={config.foodSpawnRate} min={0} max={1} step={0.05} />
            <Slider label="Ліміт ресурсів" param="maxFood" value={config.maxFood} min={50} max={2000} step={50} />
            <Slider label="Популяційний ліміт" param="maxOrganisms" value={config.maxOrganisms} min={10} max={1000} step={10} />
            ...
            <Slider label="Опір середовища" param="drag" value={config.drag} min={0.8} max={1.0} step={0.005} />
            <Slider label="Темп мутації" param="mutationFactor" value={config.mutationFactor} min={0.01} max={0.5} step={0.01} />
            <Slider label="Репродуктивний поріг" param="reproductionThreshold" value={config.reproductionThreshold} min={100} max={500} step={10} />
          </section>
          ...
          <section>
            ...
            <Slider label="Сепарація" param="separationWeight" value={config.separationWeight} min={0} max={10} step={0.1} colorClass="accent-blue-500" />
            <Slider label="Інтенсивність пошуку" param="seekWeight" value={config.seekWeight} min={0} max={10} step={0.1} colorClass="accent-blue-500" />
            <Slider label="Уникнення колізій" param="avoidWeight" value={config.avoidWeight} min={0} max={20} step={0.1} colorClass="accent-blue-500" />
          </section>
          ...
          <section>
            ...
            <Slider label="Масштаб тіл" param="organismScale" value={config.organismScale} min={0.5} max={3.0} step={0.1} colorClass="accent-amber-500" />
            <Slider label="Прозорість ресурсів" param="foodOpacity" value={config.foodOpacity} min={0.1} max={1.0} step={0.05} colorClass="accent-amber-500" />
            <Slider label="Масштаб ресурсів" param="foodScale" value={config.foodScale} min={0.5} max={3.0} step={0.1} colorClass="accent-amber-500" />
            <Slider label="Координатна сітка" param="gridOpacity" value={config.gridOpacity} min={0.0} max={0.5} step={0.01} colorClass="accent-amber-500" />
          </section>
          ...
          <section>
            ...
            <div className="mb-6">
              ...
              {config.graphicsQuality === 'CUSTOM' && (
                <div className="text-[8px] text-purple-400 text-center mt-1 uppercase tracking-wider">Кастомна конфігурація</div>
              )}
            </div>
